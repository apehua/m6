...
class SeguidoPorLaCamara(Habilidad):
    ...
    def actualizar(self):
        umbral = 250
        suavidad_movimiento = 50.0
        camara = pilas.escena_actual().camara
        distancia_x = self.receptor.x - camara.x
        distancia_y = self.receptor.y - camara.y

        if abs(distancia_x) > umbral:
            camara.x += distancia_x  / suavidad_movimiento

        if abs(distancia_y) > umbral:
            camara.y += distancia_y  / suavidad_movimiento
